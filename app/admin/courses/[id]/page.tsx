...
export default function CourseForm() {
  ...
  const [categoriesError, setالفئاتError] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchالفئات = async () => {
      try {
        const response = await fetch('/api/categories');
        
        if (!response.ok) {
          throw new Error(`تعذر جلب الفئات: ${response.status}`);
        }
        ...
        setالفئاتError(error instanceof Error ? error.message : 'Failed to load categories');
        setالفئات([]);
      }
    };
