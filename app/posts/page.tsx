  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`خطأ HTTP! الرمز: ${response.status}`);
        }
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8" dir="rtl">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">أحدث المقالات</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">اكتشف مجموعة من المقالات والموارد المفيدة</p>
        ...
        {searchQuery && (
          <div className="mb-8 text-center">
            <p className="text-gray-600">{posts.length} {posts.length === 1 ? 'مقال' : 'مقالات'} مطابقة لعبارة «{searchQuery}»</p>
          </div>
        )}
          ...
          {posts.map((post, index) => (
            ...
            >
              <div className="relative h-48 bg-gray-100">
                {post.imageUrl ? (
                  <Image src={post.imageUrl} alt={post.title} fill unoptimized className="object-cover transition-transform duration-300 group-hover:scale-105" />
                ) : post.videoUrl ? (
                ...
                <div className="absolute bottom-4 left-4">
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white bg-opacity-90 text-gray-800 shadow-sm">
                    {post.imageUrl ? (
                      <>
                        <FiImage className="mr-1" /> صورة
                      </>
                    ) : post.videoUrl ? (
                      <>
                        <FiVideo className="mr-1" /> فيديو
                      </>
                    ) : (
                      'نص'
                    )}
              ...
              <div className="p-6 flex-grow flex flex-col">
                ...
                <p className="text-gray-600 mb-5 line-clamp-3 flex-grow">{post.description}</p>
        ...
        {posts.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-1">{searchQuery ? 'لا توجد مقالات مطابقة' : 'لا توجد مقالات حاليا'}</h3>
          </div>
        )}
      </div>
    </div>
